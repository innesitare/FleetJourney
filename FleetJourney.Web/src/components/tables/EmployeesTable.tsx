...
const EmployeesTable = () => {
    ...
    return (
        <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
            <div className={`max-w-full overflow-x-auto ${showCreateEmployeeWindow || showUpdateEmployeeWindow  ? "blur" : ""}`}>
                        ...
                        <td colSpan={7} className="py-5">
                            <div className="flex justify-center">
                                <button
                                    ...
                                    onClick={() => setShowCreateEmployeeWindow(true)}
                                >
                                    +
                                </button>
                            </div>
            ...
            {showCreateEmployeeWindow && (
                <CreateEmployeeWindow
                    onEmployeeCreated={handleCreateEmployee}
                    onClose={() => setShowCreateEmployeeWindow(false)}
                />
            )}
            {showUpdateEmployeeWindow && selectedEmployee && (
                <UpdateEmployeeWindow
                    employee={selectedEmployee}
                    onEmployeeUpdated={handleUpdateEmployee}
                    onClose={() => setShowUpdateEmployeeWindow(false)}
                />
            )}
        </div>
    );
