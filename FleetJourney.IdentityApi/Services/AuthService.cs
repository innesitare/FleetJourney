...
internal sealed class AuthService : IAuthService<ApplicationUser>
{
    ...
    public AuthService(SignInManager<ApplicationUser> signInManager,
        ITokenWriter<ApplicationUser> tokenWriter,
    ...
    public async Task<string?> RegisterAsync(ApplicationUser user, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("Unexpected error occurred while signing in with password using {@Username} username, IsNotAllowed-{@IsNotAllowed}",
        ...
        await sendEndpoint.Send<RegisterNewEmployee>(new
        {
            Name = user.UserName,
            user.Email
        }, cancellationToken);
